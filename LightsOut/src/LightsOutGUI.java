 ...
 *
 * The code in this class assumes that methods in LightsOut.java
 * are working.
 */
public class LightsOutGUI {
    ...
    private class BoardPanel extends JPanel {
        ...
        public int getRowHeight() {
            int rows = LightsOut.numRows(board);
            return rows != 0 ? getHeight() / rows : 0;
        }

        public int getColumnWidth() {
            int cols = LightsOut.numCols(board);
        ...
        public void paintComponent( Graphics g ) {
            ...
            for(int row = 0; row < LightsOut.numRows(board); row++) {
                for (int col = 0; col < LightsOut.numCols(board); col++) {
    ...
    private void doMouseClick(int x, int y) {
        ...
        int toggled = LightsOut.toggleLightWithNeighbors(board, row, column);
        if (toggled > 0) {
            moves++;
        } else {
    ...
    private void updateStatusLabel() {
        String solveMsg = LightsOut.isSolved(board) ? "SOLVED! " : "";
    ...
    private void newGame(int rows, int cols, boolean empty) {
        boolean[][] grid = empty ?
                LightsOut.makeEmptyGrid(rows, cols) :
                LightsOut.makeRandomPuzzle(rows, cols);
        newGame(grid);
    }
    ...
    private LightsOutGUI() {
        this(LightsOut.DEFAULT_ROWS, LightsOut.DEFAULT_COLS);
    }

    private LightsOutGUI(int numRows, int numCols) {
        this(LightsOut.makeRandomPuzzle(numRows, numCols));
    }
    ...
    private LightsOutGUI(boolean[][] gameBoard) {
        ...
        int numRows = LightsOut.numRows(gameBoard);
        int numCols = LightsOut.numCols(gameBoard);
    ...
    public static void main(String[] args) {
        showGUI(LightsOut.makeRandomPuzzle(LightsOut.DEFAULT_ROWS, LightsOut.DEFAULT_COLS));
    }
}
